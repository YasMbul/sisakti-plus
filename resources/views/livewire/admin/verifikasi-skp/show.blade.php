      ...
      <div class="border-border-custom rounded-2xl border bg-white p-6 shadow-sm">
         ...
         <div class="mb-8 flex items-center gap-3">
            <a
               ...
               class="hover:text-primary text-black transition"
            >
            ...
            <h3 class="text-base font-bold text-black">{{ $user->name }} – {{ $user->nim }}</h3>
         </div>
         ...
         <div class="flex flex-1 overflow-x-auto">
            <table class="w-full border-collapse text-left">
               <tbody>
                  {{-- @dump ($pendingSkps) --}}
                  @forelse ($pendingSkps as $skp)
                     <tr
                        class="border-border-custom/50 hover:bg-border-custom/5 border-b transition last:border-none"
                     >
                        {{-- Nama Sertifikat/Kegiatan --}}
                        <td class="w-1/3 py-5 pr-4 text-sm font-bold text-black">
                           {{ $skp->name }}
                        </td>
                        ...
                        <td class="px-4 py-5 text-xs">
                           <span
                              class="bg-border-custom/20 text-subtext-dark-grey flex items-center justify-center rounded-full px-3 py-1 text-center font-medium"
                           >
                              {{ $skp->skpDetail->unsur->name }}
                           </span>
                        </td>

                        {{-- Tanggal Kegiatan --}}
                        <td class="text-subtext-dark-grey px-4 py-5 text-xs font-medium">
                           {{ $skp->start_date->format('d M Y') }}
                        </td>

                        {{-- Jumlah SKP atau atur dah biar ada if else biar dia kalo ditolak jadi -, kalo disetujui ada tanda plusnya --}}
                        <td
                           class="py-5 px-4 text-xs font-bold {{ $skp->status === 'approved' ? 'text-status-green' : ($skp->status === 'rejected' ? 'text-red-500' : 'text-subtext-dark-grey') }}"
                        >
                           {{ $skp->skpDetail->bobot }}
                        </td>

                        {{-- LOGIC KONDISIONAL BADGE STATUS --}}
                        <td class="px-4 py-5 text-xs font-bold">
                           @if ($skp->status == 'pending')
                              <span
                                 class="bg-status-yellow/10 text-status-yellow/80 rounded-full px-3 py-1"
                                 >Pending</span
                              >
                           @elseif ($skp->status == 'approved')
                              <span class="text-status-red/80 rounded-full bg-red-50 px-3 py-1"
                                 >Ditolak</span
                              >
                           @elseif ($skp->status == 'rejected')
                              <span
                                 class="bg-status-green/10 text-status-green rounded-full px-3 py-1"
                                 >Disetujui</span
                              >
                           @endif
                        </td>
                           ...
                           <div class="flex gap-2">
                              @if ($skp->status == 'pending')
                                 <button
                                    ...
                                    class="bg-status-green/10 text-primary hover:bg-status-green/20 cursor-pointer rounded-md px-3 py-1.5 text-xs font-semibold transition"
                                 >
                                    Setujui
                                 </button>
                                 <button
                                    wire:click="openRejectModal({{ $skp->id }})"
                                    class="text-status-red cursor-pointer rounded-md bg-red-50 px-3 py-1.5 text-xs font-semibold transition hover:bg-red-100"
                                 >
                                    Tolak
                                 </button>
                              @else
                                 ...
                                 <button
                                    disabled
                                    class="bg-status-green/10 text-primary/70 cursor-not-allowed rounded-md px-3 py-1.5 text-xs font-semibold opacity-50"
                                 >
                                    Setujui
                                 </button>
                                 <button
                                    disabled
                                    class="text-status-red/70 cursor-not-allowed rounded-md bg-red-50 px-3 py-1.5 text-xs font-semibold opacity-50"
                                 >
                                    Tolak
                                 </button>
                              @endif
                        ...
                        <td class="py-5 pl-4 text-right">
                           <a
                              wire:navigate
                              href="{{ route('admin.verifikasi-skp.show.detail', ['user_id' => $userId, 'skp_id' => $skp->id]) }}"
                              class="bg-primary hover:bg-opacity-90 text-start flex cursor-pointer items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold text-white transition"
                           >
                  ...
                  @empty
                     <td class="">
                        <x-empty-table
                           title="Tidak ada SKP yang menunggu verifikasi."
                           message="Silakan cek kembali nanti."
                        />
                     </td>
         ...
         <div class="mt-6 flex items-center justify-between pt-2">
            ...
            <button
               ...
               @disabled ($pendingSkps->onFirstPage())
            ...
            <span class="text-primary text-xs font-semibold">
               Page {{ $pendingSkps->currentPage() }} of {{ $pendingSkps->lastPage() }}</span
            >
            ...
            <button
               wire:click="nextPage"
               wire:loading.attr="disabled"
               @disabled ($pendingSkps->onLastPage())
               class="border-border-custom hover:bg-border-custom/10 cursor-pointer rounded-xl border px-4 py-2 text-xs font-bold text-black transition disabled:cursor-not-allowed disabled:opacity-50"
            >
               Next
            </button>
